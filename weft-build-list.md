...
**M0-T2 Kind registry file.** `core/src/kinds/registry.ts`: export const objects for every kind in DD §33.2–33.3 — number, name, retention class (`'E' | 'D' | 'P'`), and `expirationSeconds` per class: E=6h, D=5d, P=null. Include the normative tag names (DD §33.4) as a const enum.
*Accept:* unit test asserts kind 4910 is class D, 4913–4916 are E, 4900 is P, 4902 is marked `privateOnly: true` (never published plaintext to relays — DD §35 F1), and that no kind number is duplicated.
...
**M7-T1 Headless runner.** `porch/`: Node CLI — loads a key + config (relays, contacts) from a JSON file, subscribes, runs `onQuery`/reply-relay/handshake-forwarding forever, prints the M5-T5 counters every minute. No key generation UX; keys are imported from a PWA export.
