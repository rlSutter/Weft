...
**M9-T1 BBS+ core.** Wrap the chosen BBS+ library: `issue(attributes, issuerKey) → credential`, `verify(credential, issuerPub)`, `present(credential, disclosed, predicates) → presentation`, `verifyPresentation(presentation, issuerScope) → bool`. Attributes exactly per DD §36.1 — `subject_commitment`, **`k_cred_commitment`** (Pedersen commitment to the holder-chosen per-credential secret `k_cred`, signed by the issuer without ever being seen in the clear), `tier`, `ctx`, `issued_epoch`, `expiry_epoch`, `issuer_scope_tag`. The credential-request API accepts a holder-generated `k_cred` and blinds it before sending; the issuer never receives it.
*Accept:* issue→verify roundtrip; a presentation disclosing only `tier` verifies without revealing other attributes; a tampered presentation fails; use the library's own test vectors where they exist; **unlinkability test** — two presentations from one credential share no correlatable field; **blindness test** — the issuer's view of the request contains only the commitment, not `k_cred`.

**M9-T2 k-show nullifiers.** `nullifier(rootSecret, issuerId, epoch, showIndex)` via the spec's PRF (DD §36.1); `detectDoubleSpend(presentationA, presentationB)` returns the deanonymizing witness when `showIndex` is reused within an epoch. (The k-show nullifier remains root-secret-keyed — it enforces plurality across a user's *personas* backed by one root, which is exactly the "no more than k active selves per epoch" guarantee. Only the *scope* nullifier — M9-T3 — moved to per-credential keying.)
...
**M9-T3 Scoped pseudonyms (credential-bound).** `scopeNym(kCred, scopeId)` computes `PRF(k_cred, scope_id)` per DD §36.1 (the amended, credential-bound form — **not** the earlier `PRF(root_secret, scope_id)`); `proveBoundToCredential(scopeNym, credential, kCred)` produces a ZK proof that (a) the prover knows `k_cred` opening the credential's `k_cred_commitment`, (b) the credential is valid under the target `issuer_scope_tag`, and (c) `scopeNym` is correctly derived from `k_cred` and `scopeId`. `kCredLifecycle` helpers: `generateKCred()` (client-side, at credential request), `storeForCell(kCred, cellId)` into the backup blob, `discardForCell(cellId)` on leave (§36.1 lifecycle).
*Accept:* same credential → same nym within a scope, different nyms across scopes; two different credentials → different nyms; the binding proof verifies against the credential and **fails** when constructed by a party who holds the credential public data but not `k_cred` (this is the property that gives *unlinkability from issuance*); a client that calls `discardForCell` cannot later reproduce the nym from root-secret compromise alone.
...
**M10-T2 Join flow (blind issuance — DD §36.2).** Inner kinds **4932** (join request: in-scope credential presentation + `scope_nym` + a fresh joiner-supplied ephemeral pubkey `p_join_eph` + a cell-scoped `p_sign = derive_pk(k_sign)` where `k_sign = PRF(k_cred, scope_id ‖ "sign")` — **no joiner identity pubkey**) and **4933** (membership grant: wrapped group key + roster update, gift-wrapped to `p_join_eph`, **not** the joiner's identity key); **4922** consent receipt signed by `k_sign`, not the joiner's identity key. Consent precedes key delivery. The joiner subscribes to `p_join_eph` immediately after publishing 4932 and stays subscribed until 4933 arrives.
*Accept:* sim: a holder of a valid in-scope credential joins; a holder without one is rejected; a `scope_nym` collision (already-present or ejected) is rejected; the joiner signs the charter before receiving the key; **blind-issuance test** — the greeter's view of the 4932 event and the 4933 delivery contains no field whose value is the joiner's identity pubkey; a captured greeter that logs everything it sees cannot build a real-identity → `scope_nym` mapping.
