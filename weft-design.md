...
| 7 | `chp` | bytes, 32 B | Charter pointer = Nostr event id of the pinned charter (the id *is* its SHA-256, so pointer and integrity hash are one field; fetched from `relays`) |
| 8 | `sig` | bytes, 64 B | BIP-340 signature by `inv` over fields 0–7 |

Raw total ≈ **300 bytes → ~400 base64url chars**: QR version ~13 at EC-M, printable on a business card. The charter pointer doubles as the **cell identifier** throughout the protocol — a cell *is* its charter lineage.
