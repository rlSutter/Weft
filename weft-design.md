...
**Group-layer amendments (landed post-§36).** Two corrections surfaced during the GROUPS.md review and are now normative: (A) §36.1's scoped pseudonym is a **credential-bound nullifier** `PRF(k_cred, scope_id)` with `k_cred` a per-credential secret committed in the credential and proven in ZK — *not* the earlier `PRF(root_secret, scope_id)`, which made unlinkability-from-issuance and bounded-compromise conditional on root-secret secrecy; and (B) §36.2 now specifies **greeter blind issuance** (ephemeral delivery pubkey `p_join_eph`, cell-scoped signing key `k_sign` derived from `k_cred`, no joiner identity key on the join path), closing the group-layer form of F7. Both carry a stated relay/timing residual tied to F8.
...
**Attributes in a Weft vouch credential** (the messages signed): `subject_commitment` (a Pedersen commitment to the subject's root secret, so the holder can prove they are the subject without revealing which key), `k_cred_commitment` (a Pedersen commitment to a holder-chosen per-credential secret `k_cred`, used to derive scoped pseudonyms and cell-scoped signing keys — see *Scoped pseudonyms* below; the issuer signs the commitment without ever seeing `k_cred`), `tier` (1/2/3), `ctx` (context code), `issued_epoch`, `expiry_epoch`, `issuer_scope_tag` (a tag identifying the issuer's set — cell or region — *without* identifying the issuer). This is the credential form of the 4902 attestation (§33.3): a plaintext 4902 is delivered privately for named use; its BBS+ form is issued for anonymous use. One vouch, two representations, same underlying act.
...
**Scoped pseudonyms / accountability nullifiers (§18.2).** For a given scope (a group or rendezvous), a member's stable identifier is a **credential-bound nullifier** proven in zero knowledge:

```
scope_nym = PRF(k_cred, scope_id)
```

where `k_cred` is a **per-credential secret** the holder chooses at credential-request time (it joins the attribute list above as a committed attribute). A Pedersen commitment to `k_cred` is signed by the issuer as one of the credential's committed attributes, so the issuer signs a commitment to `k_cred` **without ever seeing `k_cred` in the clear**. At each presentation the holder proves in ZK: (a) knowledge of the opening of the committed `k_cred`, (b) that the credential is valid and unrevoked under the target `issuer_scope_tag`, and (c) that `scope_nym` is correctly derived as `PRF(k_cred, scope_id)`. This is the same nullifier machinery as k-show, keyed by scope instead of show-index. Ejection names the `scope_nym`; the holder cannot mint a second face for that scope because the derivation is deterministic in `k_cred` and re-showing there collides.

The four properties this must hold, and why the construction gives them:
- *Determinism within scope* — same credential, same scope, same nullifier, so ejection sticks.
- *Unlinkability across scopes* — a different `scope_id` yields PRF outputs indistinguishable to observers under standard PRF assumptions.
- *Unlinkability from issuance* — the issuer never learns `k_cred`, so cannot compute `scope_nym` for any scope; the best an issuer can do is bound the anonymity set to "credentials I issued whose holders could plausibly join this cell." This is what makes greeter-driven join flows safe (see §36.2, *Greeter blind issuance*).
- *Bounded compromise* — `k_cred` is per-credential state on the holder's device, **not** derived from the root secret. A client discards `k_cred` on leaving a cell, so a later root-secret compromise cannot retroactively delink cells the user has already left. (Current-membership `scope_nyms` remain recoverable on active-device compromise, which is unavoidable and is not what this property protects.)

> **Why not `PRF(root_secret, scope_id)`.** An earlier compressed form derived the nullifier directly from the long-lived root secret and a *public* `scope_id`. That silently made all four properties above conditional on root-secret secrecy — a stronger assumption than the design makes anywhere else: any party who ever obtained the root (subpoena, breach, session compromise, or the issuer itself) could compute every past, present, and future `scope_nym` for the user, and nothing could be forgotten on leave. The safety-carrying object was never the PRF; it is the ZK binding on a per-credential secret, which that phrasing elided. The credential-bound form above is normative.

**`k_cred` lifecycle (client obligations).** `k_cred` is per-credential and not root-derivable, so: (1) the encrypted backup blob (§9.2) stores the `k_cred` values for the user's *current* memberships alongside the root, so social recovery restores active cells (re-credentialing instead would strand the membership — the roster would reject the resulting fresh `scope_nym` as a stranger); (2) on leaving a cell, a well-behaved client **discards** that cell's `k_cred` and drops it from the backup blob — this is the *bounded compromise* property and is a client discipline, not a protocol enforcement (a client that hoards `k_cred` weakens only its own user's forgetting, never anyone else's). Pedersen commitments over BLS12-381 are native to BBS+'s committed-message support, so `k_cred` adds no dependency beyond the credential engine already specified.
...
**Greeter blind issuance.** Amendment A guarantees the issuer cannot compute `scope_nym` from a `k_cred` it never sees — but that guarantee is undone if issuance-time *metadata* carries the joiner's real network identity through the greeter's view. So the join path is wire-blinded: in steps (2)–(3) the greeter learns the `scope_nym` (public within the group) and verifies the ZK presentation, but MUST NOT learn the joiner's real network pubkey (the long-lived key used for asks, invites, and pairwise channels).

- The 4932 join request carries a **fresh joiner-supplied ephemeral pubkey** `p_join_eph` as the delivery address — never the joiner's identity key.
- The 4933 membership grant is gift-wrapped (kind 1059) to `p_join_eph`. The joiner decrypts with the matching ephemeral secret and stores the group key under the cell's context. (The joiner subscribes to `p_join_eph` immediately after publishing 4932 and stays subscribed until 4933 arrives.)
- The 4922 consent receipt is signed by a **cell-scoped signing key derived from `k_cred`**: `k_sign = PRF(k_cred, scope_id ‖ "sign")`, verified against `p_sign = derive_pk(k_sign)` published in the join event — not the joiner's identity key. Deriving from `k_cred` rather than a fresh per-cell keypair keeps all per-cell secrets recoverable from the one `k_cred` already in the backup blob (§36.1 lifecycle).

The greeter therefore sees only: an ephemeral requester `p_join_eph`, a valid ZK credential proof, a `scope_nym`, and a `p_sign`. **No wire field carries the joiner's identity pubkey through the greeter's view**, so a captured or curious greeter cannot build the real-identity → `scope_nym` table that scoped pseudonyms exist to prevent. Identity may still enter a cell later by a member's own choice, via the ordinary pairwise consent handshake (§5) run peer-to-peer inside the channel — this blinds *forced* linkage at join, not *voluntary* reveal afterward. This is the group-layer form of §35 F7 (invite-tree capture); without it, the credential machinery of Amendment A protects nothing on the join path.

*Residual (stated, not fixed): the greeter still observes join **timing** and the **relay** that carried the 4932. A joiner who publishes from the relay set associated with their identity leaks a probabilistic link no wire-level blinding removes; publishing the 4932 from a fresh or shared relay reduces it. This is the join-path instance of the porch/relay metadata residual (§35 F8) and is out of scope for the credential construction itself.*
...
- **Small groups (≤150 members): shared group key with per-member key-wrapping (naively rotated).** A single symmetric group key that every member decrypts with; each member also holds a per-member wrapping key established at join, used only to wrap the shared key for delivery. (This is deliberately *not* the "sender-keys" scheme in the Signal/MLS sense, where each sender ratchets their own key — a distinction worth keeping so implementers don't reach for that library.) Rotation (**4921**) publishes the new group key wrapped once per *remaining* member — O(n) ciphertexts, fine at Ostrom scale. This is the v2-initial implementation.
...
| 4932 | **Group join request** | E | Credential presentation proving in-scope backing + the joiner's `scope_nym` + a fresh ephemeral delivery pubkey `p_join_eph` + a cell-scoped `p_sign`, wrapped to a steward/greeter. Carries **no** joiner identity key (blind issuance, §36.2). | §36.2 |
| 4933 | **Membership grant** | D | Current group key (or MLS `Welcome`) wrapped to **`p_join_eph`** (not the joiner's identity key) + roster update; issued after 4932 verifies. | §36.2 |
